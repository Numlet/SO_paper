...
#            continue
        ...
        cube_CTT=iris.load(run_path[key]+'A_TRAIN_COLLOCATED/L1_CTT_Cloud_top_temperatureA-train_collocated.nc')[0][:,:]
        ...
        cube_CTT=stc.clean_cube(iris.load(run_path[key]+'A_TRAIN_COLLOCATED/L1_CTT_Cloud_top_temperatureA-train_collocated.nc')[0][:,:])




    try:
        np.save(pspc_fol+'CTT_distribution_'+key,cube_CTT.data)
    except:
        np.save(pspc_fol+'CTT_distribution_'+key,cube_CTT.data.data)

    CTT_dict[key]=np.nanmean(cube_CTT.data)
#    masked_cube[mask]=np.nan
    try:
        np.save(pspc_fol+'CTT_distribution_filtered_'+key,cube_CTT.data)
    except:
        np.save(pspc_fol+'CTT_distribution_filtered_'+key,cube_CTT.data.data)
...
plt.savefig(sav_fol+'CTT_barplot.png')
