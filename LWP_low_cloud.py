...
#        cube = cube.regrid(sample_cube, iris.analysis.Linear())
        ...
        cube_SW=stc.clean_cube(iris.load(run_path[key]+'A_TRAIN_COLLOCATED/All_time_steps_m01s01i208_toa_outgoing_shortwave_fluxA-train_collocated.nc')[0])[itime,:,:]
...
#LWP_dict['C3_VT17_MEAN']
for cloud in list_clouds:
    LWP_dict[cloud+'_SATELLITE']=np.nanmean(LWP_satellite_dict[cloud+'_SATELLITE'])
    LWP_dict_filtered[cloud+'_SATELLITE']=np.nanmean(LWP_satellite_dict[cloud+'_SATELLITE'])
...
for param in list_params:
    ...
    if param=='GLOBAL':param='GLOBAL MODEL'
...
ax.set_title('b) Liquid Water Path')
...
plt.savefig(sav_fol+'LWP_barplot.eps')

#%%

plt.figure()
data=np.load(pspc_fol+'LWP_distribution_filtered_C3_VT17_MEAN.npy')
