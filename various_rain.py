...
#                cube=stc.clean_cube(iris.load(ukl.Obtain_name(run_path[name]+'L1/',var))[0],clean_value)
                cube=stc.clean_cube(iris.load(ukl.Obtain_name(run_path[name]+'All_time_steps/','RAIN_NUMBER'))[0],clean_value)
...
#            rain=cube.data.mean(axis=(1,2))[1:]
            ...
            plt.yscale('log')
            plt.xlabel('time')
            plt.ylabel('rain_number')
            plt.title(cloud+' rain_number')
        plt.legend(loc='best')
        icol=icol+1
        plt.savefig('/nfs/see-fs-01_users/eejvt/CASIM_hydrometeors/'+cloud+'rain_number.png')
#            plt.legend()
