...
def clean_cube(cube,value=100):
    if len(cube.shape)==4:
        ...
        new_cube=new_cube[:-value,:-value]
    return new_cube
stc.clean_cube=clean_cube

SW_dict_mask=OrderedDict()
SW_dict_data=OrderedDict()
SW_dict_filtered_data=OrderedDict()
...
#plt.subplot(3,3,7)
#plt.title(np.nanmean(SW_dict_filtered_data['C1_SATELLITE']))
#plt.imshow(SW_dict_data['C1_SATELLITE'],vmin=0,vmax=850)
#plt.colorbar()

plt.subplot(3,3,8)
plt.title(np.nanmean(SW_dict_filtered_data['C3_SATELLITE']))
plt.imshow(SW_dict_data['C3_SATELLITE'],vmin=0,vmax=750)
plt.colorbar()
#plt.subplot(3,3,9)
#plt.title(np.nanmean(SW_dict_filtered_data['C3_SATELLITE']))
#plt.imshow(SW_dict_data['C3_SATELLITE'],vmin=0,vmax=850)
#plt.colorbar()
...
#cube_DM10 = stc.clean_cube(iris.load(ukl.Obtain_name(sim_path+'/DM10/A_TRAIN_COLLOCATED/','m01s01i208'))[0])
#cube = stc.clean_cube(iris.load(ukl.Obtain_name(sim_path+'/GLO_MEAN/A_TRAIN_COLLOCATED/','m01s01i208'))[0])
#
##SW_dict[C1]
#
...
#cube = stc.clean_cube(iris.load(ukl.Obtain_name('/nfs/a201/eejvt/CASIM/SO_KALLI/TRY2/ALL_ICE_PROC/A_TRAIN_COLLOCATED/','m01s01i208'))[0])
##cube_DM10 = stc.clean_cube(iris.load(ukl.Obtain_name(sim_path+'/DM10/All_time_steps/','m01s01i208'))[0])
#
#reload(stc)
#model_lons,model_lats=stc.unrotated_grid(cube)
...
#X,Y=np.meshgrid(model_lons, model_lats)
##Xo,Yo=np.meshgrid(lon_old,lat_old)
##data_old= sc.interpolate.griddata(coord_model, cube_oldm.data.flatten(), (fX,Y), method='linear')
##grid_z0 = sc.interpolate.griddata(coord, sat_SW, (X,Y), method='nearest')
#grid_z1 = sc.interpolate.griddata(coord, sat_LW, (X,Y), method='linear')
#
...
#cube=iris.load_cube(run_path['C3_VT17_MEAN']+'All_time_steps/All_time_steps_m01s01i208_toa_outgoing_shortwave_flux.nc')
#data=0.5*cube[12,].data+0.5*cube[13,].data
#SW_dict_filtered['C3_VT17_MEAN']=data.mean()
...
##%%
#data=np.load(pspc_fol+'SW_distribution_C3_VT17_MEAN.npy')
#data_filtered=np.load(pspc_fol+'SW_distribution_filtered_C3_VT17_MEAN.npy')
#
#plt.figure()
#plt.subplot(3,1,1)
#plt.imshow(data)
